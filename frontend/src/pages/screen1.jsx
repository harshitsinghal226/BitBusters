...
export default function Screen1({ onProfilePhotoClick }) {
  ...
  const filteredUsers = mockUsers.filter((user) => {
    ...
    return (
      matchesSearch && matchesAvailability && user.profileType === "Public"
    );
  });
      ...
      <div className="w-full max-w-3xl rounded-2xl bg-black/70 shadow-2xl p-6 md:p-8 border border-purple-900">
        ...
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-6">
          ...
          <div className="flex gap-4 items-center">
            <button className="text-purple-400 underline underline-offset-4 hover:text-purple-300 font-semibold">
              Swap request
            </button>
            ...
            >
              <div className="flex items-center gap-4 flex-1">
                <div
                  ...
                  onClick={() =>
                    onProfilePhotoClick && onProfilePhotoClick(user)
                  }
                >
                  {user.photo ? (
                    <img
                ...
                <div>
                  ...
                  <div className="flex flex-wrap gap-2 mt-1">
                    {user.skillsOffered.map((skill) => (
                      <span
                        key={skill}
                        className="bg-purple-800/70 border border-purple-500 rounded-full px-2 py-0.5 text-xs text-purple-100"
                      >
                        {skill}
                      </span>
                    ))}
                  ...
                  <div className="flex flex-wrap gap-2 mt-1">
                    {user.skillsWanted.map((skill) => (
                      <span
                        key={skill}
                        className="bg-black/40 border border-purple-500 rounded-full px-2 py-0.5 text-xs text-purple-300"
                      >
                        {skill}
                      </span>
                    ))}
              ...
              <div className="flex flex-col items-end gap-2">
                <button className="bg-cyan-600 hover:bg-cyan-500 text-white px-4 py-2 rounded font-semibold transition-colors">
                  Request
                </button>
              </div>
            </div>
